...
class ServerTunnel: Tunnel, TunnelDelegate, StreamDelegate {
		...
		repeat {
			...
			if packetBytesRemaining == 0 {
                ...
                if packetBuffer.count == MemoryLayout<UInt32>.size {
                    ...
                    packetBuffer.copyBytes(to: withUnsafeMutablePointer(to: &totalLength, {$0.withMemoryRebound(to: UInt8.self, capacity: 1, {$0})}), count: MemoryLayout.size(ofValue:totalLength))
    ...
    override func closeTunnel() {
        ...
        if let stream = readStream {
            ...
            let socketData = CFReadStreamCopyProperty(stream, .socketNativeHandle) as? NSData
