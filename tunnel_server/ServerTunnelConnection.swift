...
class ServerTunnelConnection: Connection {
	...
	func createTUNInterface() -> Int32 {
        ...
        let socketAddr = withUnsafePointer(to: &socketAddressControl, {$0.withMemoryRebound(to: sockaddr.self, capacity: 1, {$0})})
        let connectResult = connect(utunSocket, socketAddr, socklen_t(MemoryLayout.size(ofValue:socketAddressControl)))
